 ...
 * each has to be refused for reasons the player needs spelled out ("you are
 * already in Daily Scribble Cup #4", "check-in has closed"), and a
 * fire-and-forget socket event has nowhere to put that.
 *
 * ## Why a channel rather than a global broadcast
 *
 * `io.emit` reaches every socket in the process, including the ones mid-round
 * with a canvas to keep up with. A player in a match has no use for "slot 2
 * opened registration", and a drawing turn is the last place to spend
 * bandwidth on it. The channel means the fan-out reaches the screens that are
 * actually showing a tournament.
 *
